...
export class HomeComponent implements OnInit {
  ...
  async setOption(option, cat) {
    /*if(cat.NoAction === '1')
      return;
    */
    const locationObj = await this.storage.get("location");

    console.log("option", option);
    console.log("category", cat);

    const is_classifieds = cat?.is_classifieds ?? "0";

    const classified_category = cat?.classified_category ?? "0";

    const sort_by_name = cat?.SortSubcategories ?? "0";

    const number_of_entries_to_display =
      cat?.number_of_entries_to_display ?? "10";
    ...
    switch (cat.action_type) {
      ...
      default:
        ...
        this.router.navigateByUrl(
          `/folder/${locationObj.qpId}/${cat.qpId}/${cat.cat_name}?is_classifieds=${is_classifieds}&classified_category=${classified_category}`+
            `&sort_by_name=${sort_by_name}&number_of_entries_to_display=${number_of_entries_to_display}`
        );
        break;
    }
  }
    ...
    this.locationService
      ...
      .subscribe((data: Array<any>) => {
        if (data.length > 0) {
          ...
          this.location.picture_home = loc.picture_home
            ? loc.picture_home
            : null;
              ...
              this.storage.get("categories").then(async (val) => {
                if (
                  ...
                  let base: string = String(
                    await this.getBase64ImageFromUrl(this.location.picture_home)
                  );
                  ...
                  this.categoryService
                    ...
                    .subscribe(async (cats: Array<any>) => {
                      ...
                      cats = cats.filter((value) => {
                        for (var i = locationCats.length - 1; i >= 0; i--) {
                          ...
                          if (Number(locationCats[i].category) == value.qpId) {
                            ...
                            value.is_classifieds =
                              locationCats[i].is_classifieds ?? "0";
                      ...
                      for (var i = this.categorys.length - 1; i >= 0; i--) {
                        this.categorys[i].viewId =
                          "card" + i + "" + new Date().getTime();
                      }
  ...
  onSwipeUp() {
    ...
    if (this.selectedCard < this.categorys.length - 1) {
      this.setOption(
        this.selectedCard + 1,
        this.categorys[this.selectedCard + 1]
      );
    }
  }
  ...
  scrollCustomImplementation(element: HTMLElement, up) {
    ...
    function showAnimation(timestamp) {
      ...
      let stop = false;
      if (!up) stop = pos >= firstPos + target;
      else stop = pos <= firstPos + target;
  ...
  private getCloseLocation(data: Array<any>, lat, long) {
    ...
    for (var i = 0; i < data.length; ++i) {
      let dist = this.getDistanceFromLatLon(
        Number(data[i].Lat),
        Number(data[i].Long),
        lat,
        long
