...
export class BusinessDetailComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.sponsoredLabel = await this.settingsService.getValue(
      this.settingsService.SPONSORED_LABEL
    );
    ...
    this.is_classifieds =
      this.activatedRoute.snapshot.queryParamMap.get("is_classifieds") ?? "0";
    ...
    this.businessPropertiesService
      ...
      .subscribe((properties: any) => {
        ...
        this.businessService
          .getBusinessById(this.id, this.device.uuid)
          .subscribe((data: any) => {
            if (data) {
              const bodyImage = data.body_image;
              if (bodyImage && bodyImage != "") {
                data.body_image =
                  bodyImage.substring(0, bodyImage.lastIndexOf(".")) + ".webp";
              }
              ...
              this.businessPropertiesService
                .getBusinessPropertiesByBusiness(this.bus.qpId)
                .subscribe((props: Array<any>) => {
                  ...
                  for (let i = 1; i <= 8; i++) {
                    ...
                    if (
                      this.bus[mediaLink] &&
                      this.bus[mediaLinkType] &&
                      this.bus[mediaLinkLabel]
                    ) {
                      console.log("mediaLinkType", this.bus);
                      const property = this.PROPERTY_TYPE.find(
                        (p) => p.qpId == this.bus[mediaLinkType]
                      );
                  ...
                  for (let i = 0; i < propArray.length; i++) {
                    let p = propArray[i];
                    if (p.property.toLowerCase() === "menu") p.position = 1;
                    else if (p.property.toLowerCase() === "url") p.position = 2;
                  ...
                  if (currentUser != null && currentUser.uid) {
                    this.bookmarkService
                      .getBookMark(currentUser.uid, this.bus.qpId)
                      .subscribe((bookmark: Array<any>) => {
                        if (bookmark.length) {
  ...
  public getPropertyIcon(property) {
    if (property.toLowerCase() === "appointment") return "checkbox";
    else if (property.toLowerCase() === "takeout/delivery") return "basket";
    else if (property.toLowerCase() === "menu") return "restaurant";
    else if (property.toLowerCase() === "reservations") return "calendar";
    else if (property.toLowerCase() === "url") return "aperture";
    else if (property.toLowerCase() === "email") return "mail";
    else if (property.toLowerCase() === "phone") return "call";
    else if (property.toLowerCase() === "facebook") return "logo-facebook";
    else if (property.toLowerCase() === "instagram") return "logo-instagram";
    else if (property.toLowerCase() === "twitter") return "logo-twitter";
    else if (property.toLowerCase() === "youtube") return "logo-youtube";
  }
  ...
  async shareBusiness() {
    ...
    const deeplinkResponse = await this.branchService
      .shareDeeplinkByBusiness(title, this.bus)
      .toPromise();
  ...
  async openAdurl() {
    const url =
      this.bus.ad_URL != null && this.bus.ad_URL.length > 0
        ? this.bus.ad_URL
        : this.ad.Ad_Link;
    if (url != null && url.length > 0) Browser.open({ url: url });
  }

  async shareAdLink() {
    if (this.ad?.Ad_Link?.length > 0) {
      await this.socialSharing.share(
        "",
        null,
        this.ad.Ad_Image,
  ...
  async openMedia(url: string, type: string) {
    ...
    const media: BusinessMedia = {
      url,
      type:
        type === "image" ? BusinessMediaType.image : BusinessMediaType.video,
    };
  ...
  async propAction(prop) {
    if (prop.property.toLowerCase() === "email") {
      ...
      //URL, Menu, Reservations, Takeout/Delivery, Appointment
      await this.fcmService.analyticsLogEvent("screen_action", {
        page: "business_details",
        action: "open_url",
        business: this.bus.Name,
  ...
  async openBusinessUrl() {
    ...
    if (
      this.bus &&
      this.bus.Business_Logo_URL &&
      this.bus.Business_Logo_URL.length > 0
    ) {
  ...
  public async toogleBookmark() {
    ...
    if (this.bookmark) {
      ...
      this.bookmarkService
        .deleteBookmark(this.bookmark.qpId)
        .subscribe((res) => {
          this.bookmark = null;
          this.ionLoader.hideLoader();
        });
      ...
      this.bookmarkService
        .createBookmark(currentUser.uid, this.bus.qpId)
        .subscribe((res) => {
          this.bookmark = res;
          this.ionLoader.hideLoader();
        });
